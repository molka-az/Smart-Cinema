...
int stati::Statistique_partie2()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from commercial where ID BETWEEN '21' AND '31'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int stati::Statistique_partie3()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from commercial where ID BETWEEN '41' AND '100'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int stati::Statistique_partie4()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from commercial where ID BETWEEN '10' AND '40'") ;
    while(requete.next())
    {
            count++ ;
    }
